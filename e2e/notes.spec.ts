...
test('note editor saves drafts to the server only and locks notes', async ({ page }) => {
  ...
  expect(saved).toMatchObject({ client_id: 'client-1', section_1: 'Client reported improved sleep this week.', enc_version: 0, is_locked: false });
  ...
  expect(state.requests.some(r => r.method === 'PATCH' && JSON.stringify(r.body).includes('"is_locked":true'))).toBe(true);
