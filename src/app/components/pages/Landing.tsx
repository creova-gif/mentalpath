...
export function Landing() {
  ...
  const problemCards = t('problemSection.cards', { returnObjects: true }) as Array<{ badge: string; title: string; desc: string }>;
  const compliancePoints = t('complianceSection.points', { returnObjects: true }) as Array<{ badge: string; title: string; desc: string }>;
  const testimonials = t('testimonialsSection.testimonials', { returnObjects: true }) as Array<{ quote: string; name: string; role: string }>;
  const starterFeatures = t('pricingSection.plans.starter.features', { returnObjects: true }) as string[];
  const soloFeatures = t('pricingSection.plans.solo.features', { returnObjects: true }) as string[];
  const groupFeatures = t('pricingSection.plans.group.features', { returnObjects: true }) as string[];
  const billingTags = t('bento.billing.tags', { returnObjects: true }) as string[];
  const clientTemplates = t('bento.clientMgmt.templates', { returnObjects: true }) as string[];

  const COMPLIANCE_ICONS = [
    <path key="0" d="M12 2L3 7v5c0 5.5 3.8 10.7 9 12 5.2-1.3 9-6.5 9-12V7L12 2z"/>,
    <><rect key="r" x="3" y="11" width="18" height="11" rx="2"/><path key="p" d="M7 11V7a5 5 0 0110 0v4"/></>,
    <><path key="0" d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><path key="1" d="M14 2v6h6M16 13H8M16 17H8"/></>,
    <><path key="0" d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle key="c" cx="9" cy="7" r="4"/><path key="1" d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></>,
  ];

  const TESTIMONIAL_AVATARS = [
    { avatar: 'AO', avatarBg: '#d4e8e4', avatarColor: '#2a5448' },
    { avatar: 'MR', avatarBg: '#e4d8d4', avatarColor: '#543028' },
    { avatar: 'KN', avatarBg: '#d4d8e8', avatarColor: '#283054' },
  ];
    ...
    <div style={{ fontFamily: 'var(--font-body)', background: '#060d09' }}>
        ...
        <div style={{ maxWidth: 1200, width: '100%', margin: '0 auto', paddingTop: 56, paddingBottom: 96, position: 'relative', zIndex: 1 }} className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-16 items-center">
          ...
          <div className="hidden md:block" style={{ position: 'relative' }}>
            ...
            <div className="mp-float" style={{ position: 'relative', zIndex: 1, background: 'rgba(245,240,232,0.04)', border: '1px solid rgba(255,255,255,0.09)', borderRadius: 18, overflow: 'hidden', backdropFilter: 'blur(24px)', boxShadow: '0 32px 80px rgba(0,0,0,0.6), 0 0 0 1px rgba(255,255,255,0.04)' }}>
              ...
              <div style={{ padding: 22 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 18 }}>
                  {[
                    { v: '6', l: t('dashboard.stats.sessionsToday') },
                    { v: '$840', l: t('dashboard.stats.billedThisWeek') },
                    { v: '23', l: t('dashboard.stats.activeClients') },
                  ].map((s, i) => (
            ...
            <div style={{ position: 'absolute', top: -18, left: -18, background: 'rgba(74,124,111,0.18)', border: '1px solid rgba(74,124,111,0.28)', borderRadius: 12, padding: '8px 14px', backdropFilter: 'blur(16px)', zIndex: 2, display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 600, color: '#8abda8', letterSpacing: '0.3px' }}>{t('hero.dashboardCard.phipaBadge')}</span>
            </div>
          </div>
        </div>
      ...
      <div style={{ background: '#0d1c16', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }} className="grid grid-cols-1 sm:grid-cols-3">
          {[
            { stat: t('statsBar.practitioners.stat'), label: t('statsBar.practitioners.label'), sub: t('statsBar.practitioners.sub') },
            { stat: t('statsBar.price.stat'), label: t('statsBar.price.label'), sub: t('statsBar.price.sub') },
            { stat: t('statsBar.trial.stat'), label: t('statsBar.trial.label'), sub: t('statsBar.trial.sub') },
          ].map((s, i) => (
              ...
              <div style={{ flex: '1 1 240px', background: '#f2ede3', borderRadius: 12, padding: 20 }}>
                ...
                {clientTemplates.map((tmpl, i, arr) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 0', borderBottom: i < arr.length - 1 ? '1px solid rgba(74,124,111,0.1)' : 'none' }}>
                    ...
                    <span style={{ fontSize: 12, color: '#2a5448' }}>{tmpl}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            >
              ...
              >
                {t('pricingSection.plans.starter.cta')}
              </Link>
            </div>
            ...
            >
              ...
              <Link to="/onboarding" style={{ display: 'block', textAlign: 'center', padding: '13px 0', borderRadius: 10, fontSize: 15, fontWeight: 600, background: 'white', color: '#2a5448', textDecoration: 'none', boxShadow: '0 4px 16px rgba(0,0,0,0.15)', transition: 'background 0.2s' }}>
                {t('pricingSection.plans.solo.cta')}
              </Link>
            </div>
            ...
            >
              ...
              >
                {t('pricingSection.plans.group.cta')}
              </Link>
            </div>
          </div>
        </div>
      ...
      <section style={{ background: '#2a4f42', padding: '88px 5vw 100px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ display: 'flex', gap: 10, maxWidth: 480, margin: '0 auto 20px', flexWrap: 'wrap', justifyContent: 'center' }}>
            <input type="email" placeholder={t('ctaSection.emailPlaceholder')} style={{ flex: '1 1 240px', padding: '14px 18px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.16)', background: 'rgba(255,255,255,0.07)', color: '#f5f0e8', fontSize: 15, outline: 'none' }} />
